...
export const restaurantSignup = async(req, res) => {
    try {
        ...
        const credential = {name, email, password, confirmPassword, location}
        ...
        res.cookie('restaurant-auth', token, {httpOnly: true})
...
export const restaurantRemove = async(req, res) => {
    try {
        res.clearCookie('restaurant-auth', {httpOnly})

        return res.status(200).json({message: "Successfully logged out restaurant"})
